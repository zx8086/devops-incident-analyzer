...
## Post-merge confirmation replay (PR #558, merged `fff2d16d`)

One further replay against a fresh `:5174` web app started from the merged `main` checkout (not the feature branch), to confirm the fix holds on the actual code that ships, not just the pre-merge branch. `:9084` re-preflighted first (`gitlab_blast_radius` returned `definition-name-match`, confirming post-fix code). Same incident prompt, fresh threadId `f484cd4b-1410-4a62-90f3-7cf08931e3ad`, trace `019fba22-cc3d-77b1-9b50-b13309d98a3c`, responseTime 201.1s.

- **Criterion 1 (notes)**: `gitlab_get_merge_request_notes` present in `toolsUsed` and confirmed via LangSmith -- `gitlab_get_merge_request_notes(project_id:43242609, merge_request_iid:383)`, correctly targeting MR !383, the same strongest candidate identified in every prior run. **3/4 total post-fix runs now (fix2, fix3, and this post-merge run) -- criterion 1 remains solidly fixed.**
- **Criterion 2 (prior-art)**: one of five `gitlab_search` calls used `scope:"work_items"` with `search:"UnambiguousTimeoutException"` -- clean ERROR-CLASS-only vocabulary this time, no service-name mixing (an improvement over the earlier fix1 run's "UnambiguousTimeoutException styles"). The final report correctly states "gitlab_search across GitLab issues for UnambiguousTimeoutException returned zero hits -- no prior tracked issue for this exception class in this project," treating the zero-hit result as normal per the skill's design. This is the first observed run where the prior-art check fired AND used fully compliant vocabulary. Still not reliable enough to call "fixed" (this is 1 success among several total attempts across the session), but it is a positive data point that the check CAN and DOES work correctly end-to-end, including reaching the final report in the expected exclusion-rule form.
- Final report also confirms the notes exclusion rule works as designed: "MR !379 carries zero review notes per `gitlab_list_merge_requests`; no reviewer flagged Couchbase/schema impact pre-merge" -- reported as an absence of signal, not as a gap or failure.
- Cleanup: `:5174` (tracked PID 72497, child 72502) killed; `lsof -nP -iTCP:5174 -sTCP:LISTEN` empty. User's `:5173` (PID 68026) and `:9084` untouched. One prior replay attempt against this merged code was interrupted mid-run by a session disconnect (truncated to 68 SSE lines, no stray process left behind -- both the tracked PID and the :5174 listener were already gone on reconnect); this section reports the successful re-run.
