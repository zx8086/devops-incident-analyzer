...
- **History**: this is the single handover for SIO-1282. An earlier one written pre-merge at `5b3c796a` was folded into this file and deleted — nothing was lost, and there is no second doc to reconcile.
...
## What changed after SIO-1281 landed

This spike was blocked on SIO-1281 until it merged on 2026-07-29. That merge moved the ground in three ways, and every figure below is re-measured on `9c001236` rather than carried over from the pre-merge draft.

1. **The 16 files this spike must not convert are gone.** They were deleted; 14 sections were restored into `knowledge/playbook/`, and 2 eu-b2b runbooks were archived to `knowledge/_archive/eu-b2b-ilm/`. The sequencing risk that once blocked this spike is discharged.
2. **elastic-iac counts shifted.** `playbook` grew 94,289 -> **110,237 bytes**; `_archive` shrank to **13,432**; `runbooks` is **6**, unchanged. Entry count is still **46**.
