...
The epic stays open until `evidence_index.search` is seen in production logs with `hitCount > 0` on a real turn.

Read the next section before planning any work. The obvious move (run an eval) does not work, and the reason is structural.
...
### Option A: read production logs (cheapest, no code)

The `evidence_index.search` event shipped in the same PR precisely so this is answerable from logs.
...
Emitted in `packages/agent/src/evidence-index.ts` (search method). The companion truncation event carries `indexedRows`, so the two together answer the real question:

- Turns where `subagent.tool_result_truncated` has `indexedRows > 0` = turns where recovery was POSSIBLE.
- Turns that also show `evidence_index.search` = turns where the model actually reached for it.

The ratio is the adoption rate. If the denominator is healthy and the numerator is zero over real traffic, that is a decisive negative result and Option C applies.

### Option B: a targeted probe whose answer REQUIRES an elided record
...
- Web app runtime in AgentCore prod (Bun vs Node), which decides whether the evidence store needs the dual-driver path from `tool-call-metrics.ts:184-267`. Still open, unrelated to adoption.
