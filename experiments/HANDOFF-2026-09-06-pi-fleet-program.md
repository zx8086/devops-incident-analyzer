...
Status 2026-09-06: Phase 0b merged (PR #690, squash `7847dfbf`, SIO-1654 Done); PR #682 merged (`e004441c`); Phase 0 code merged (PR #691, squash `4caddf23`) following `docs/superpowers/plans/2026-09-06-pi-fleet-phase-0b-and-0.md`. Still open after #691: the `@devops-agent/pi-coms/contracts` import in the analyzer (one workspace dependency in `packages/agent/package.json`, needs approval), one `incident-analyzer` hub token per environment, the dev-bucket publish and host convergence, archiving the pi-coms repo. Phase 1 (SIO-1649) merged the same day (PR #692, squash `fc5d96cb`) per `docs/superpowers/plans/2026-09-06-pi-fleet-phase-1.md`: `agents/pi-fleet/` definitions, bridge exporter and version gate, `agent-release.yml`, bundle carries `vendor/pi-fleet/`. Phase 1b (SIO-1653) tooling merged the same day (PR #693, squash `41523088`) per `docs/superpowers/plans/2026-09-06-pi-fleet-phase-1b.md`: manifest, `just fleet` CLI, adopt mode, rendered roots, S3 backend; nothing applied to AWS (user-run with the real `deploy/fleet.yaml`). Phase 2a (SIO-1650) merged the same day (PR #694, squash `62c8a59d`) per `docs/superpowers/plans/2026-09-06-pi-fleet-phase-2a.md`: `PiFleetPane`, `/api/pi/*` routes, sliced await with browser re-polling, `PI_COMS_PANE_*`; the manual pane run against a hub and the `pi-fleet` principal are user-run. Phase 2b (SIO-1652) merged the same day (PR #695, squash `cb2fcdca`) per `docs/superpowers/plans/2026-09-06-pi-fleet-phase-2b.md`: `fetchFleetInbox` node before `aggregate` (32 nodes), `fleetInboxDigest` sidecar, `FleetInboxCard`, `PI_COMS_INBOX_*`; the corp fleet reports to `ops`. Phase 3 (SIO-1651) merged the same day (PR #696, squash `cee8c2ad`) per `docs/superpowers/plans/2026-09-06-pi-fleet-phase-3.md`: `pi-handoff.yaml` chaining the skillflow `graph` and `agent` step handlers (their first production wiring), a detached post-turn trigger gated by `PI_HANDOFF_ENABLED` (default off), `runHubTask` exported so the workflow and the SIO-1635 card share one hub path, and verdicts recorded as structured fields only. Two deviations from that phase's handover, both verified against the code and recorded in the plan and `pi-coms-verification.md`: the `graph` step READS the closing turn's `closingReport` instead of re-invoking the pipeline (re-invoking would repeat a full 7-agent fan-out to reproduce an existing report), and the workflow registers as the analyzer's own principal, so no `pi-fleet` hub token is needed. The live run with `PI_HANDOFF_ENABLED` against a hub with a registered spoke is user-run. All seven planned phases (0b, 0, 1, 1b, 2a, 2b, 3) are merged. Phase 2c, previously deferred, was then issued, planned and built the same day at the user's instruction (SIO-1655): PR #697 landed the `graphFor(agentName)` registry as a behaviour-preserving refactor (squash `756716e2`), and PR #698 the fleet console graph itself (squash `9f125775`) -- a separate in-process persona `agents/pi-fleet-console/`, five hub tools, and `wrapUntrusted`, the one place a hub reply may reach a model. Gated off by `PI_FLEET_GRAPH_ENABLED`; the live run against a hub with registered spokes is user-run. The whole pi-fleet programme is now built.
