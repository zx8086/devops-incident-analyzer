...
1. **Read live cluster state before writing the diff.** Use `elasticsearch_cloud_get_deployment`, `..._get_plan_history`, `..._get_cluster_health` for the target cluster. Do not rely on tracker rows or memory snapshots alone.
