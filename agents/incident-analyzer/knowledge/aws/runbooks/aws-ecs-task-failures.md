...
If `stoppedReason` mentions `unable to assume role`, this is an IAM trust-policy issue, not a networking problem. For an ECS task role, check that the trust policy's principal is `ecs-tasks.amazonaws.com` and that any `aws:SourceAccount`/`aws:SourceArn` conditions match this account and cluster -- `ExternalId` does not apply here (that condition is specific to the AgentCore cross-account role documented in [`aws-iam-permission-troubleshooting.md`](./aws-iam-permission-troubleshooting.md#trust-policy-issues), a different role entirely).

### 8. Correlate against the rollout history
If the failures correlate with a recent service revision (deployment), the task definition itself is the likely culprit. Service events will reference `taskDefinition: arn:aws:ecs:...:task-definition/<family>:<rev>`. Use `aws_ecs_describe_task_definition` on both the failing revision and the previous one to compare image-tag changes and memory/CPU allocations directly rather than inferring them from event text. For environment configuration, compare `containerDefinitions[].environment[].name` and `containerDefinitions[].secrets[].name`/`valueFrom` (which references, not resolves, a Secrets Manager or SSM value) -- **never** include the plaintext `environment[].value` field in a finding or log; `DescribeTaskDefinition` returns it unredacted to anyone with read access, and a misconfigured task can carry real secrets there instead of in `secrets`.
