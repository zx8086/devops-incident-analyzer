...
- **Reverse IP -> service resolution (SIO-1200)** (a bare internal IP appears in logs, error text, or another datasource's finding and its owning workload is unknown). This is the reverse of the Network path protocol above (workload -> egress); here you start from the IP alone:
    0. **KG cache hint (verify-then-trust, SIO-1205).** When the supervisor focus block carries a knowledge-graph cache hint ("KG cache: `<ip>` last bound to `<arn>` ..."), treat it as a STARTING POINT, not an answer: it tells you which workload to check first, but bindings go stale. Still run the live confirm (step 1); if the live answer contradicts the cache, report the contradiction explicitly ("KG cache said `<arn>`, live lookup shows `<other>`") so the stale binding gets corrected. Never skip the live confirm silently.
...
- **Network map protocol (SIO-1205)** (how does traffic REACH the focus service from the edge — DNS record -> ALB/NLB -> listener -> target group -> target health). The ingress mirror of the Network path protocol above (which traces egress); run it when the incident implicates the entry path or when building the incident network map:
    1. `aws_route53_list_hosted_zones`, then `aws_route53_list_resource_record_sets` for the zone covering the focus service's domain — pass `startRecordName` near the focus service hostname instead of walking the whole zone (record-set pagination is positional: continue with the response's `NextRecordName`/`NextRecordType` as `startRecordName`/`startRecordType`, never a cursor).
    2. Match the A/ALIAS/CNAME record targets against `aws_elbv2_describe_load_balancers` `DNSName` values. Normalize case and trailing dots before comparing (Route53 names end in `.`; LB DNSNames do not).
    3. Follow the matched LB into `aws_elbv2_describe_listeners` (its `DefaultActions[].TargetGroupArn` is the listener-to-target-group link) -> `aws_elbv2_describe_target_groups` -> `aws_elbv2_describe_target_health`, capped to the focus-relevant target groups — target health is one call per target group, so do NOT sweep every target group of an unrelated LB. `Target.Id` in the health rows is an instance id or an IP; resolve IPs via `aws_ec2_describe_network_interfaces` and place them in subnets via `aws_ec2_describe_subnets` (CIDR match).
...
- **Fleet-wide top-N triage (Metrics Insights).** Two sibling metric tools; pick by whether the resource is known:
    ...
    - **Query library (copy these verbatim, substitute `<...>` only).** Grammar rules: ONE query per call; string values in SINGLE quotes; WHERE supports only `=`, `!=`, `AND` (no OR/LIKE/IN); `LIMIT` <= 500; window <= 14 days (`startRelative` defaults to `now-3h`; pass `now-14d` max).
        ...
        7. Top-10 MSK consumer groups by lag (dimension names with spaces are double-quoted; never add "Broker ID" to a lag query): `SELECT MAX(SumOffsetLag) FROM SCHEMA("AWS/Kafka", "Cluster Name", "Consumer Group", Topic) GROUP BY "Consumer Group", Topic ORDER BY MAX() DESC LIMIT 10`
...
- Logs: `aws_logs_describe_log_groups` (find the group) -> `aws_logs_start_query` -> `aws_logs_get_query_results` (Insights polling pattern).
    ...
    - **queryString syntax (copy this).** A known-good Logs Insights query to find a service's errors: `fields @timestamp, @message | filter @message like /THE1/ | sort @timestamp desc | limit 20`. Chain commands with `|`; use `filter @message like /regex/` for text. A `MalformedQueryException` ("unexpected symbol", "invalid syntax", "query definition snippets") is a query-string error, not a window error: follow the recovery sequence under `MalformedQueryException` recovery below.
    - **Logs Insights query library (copy these verbatim, substitute `<...>` only).** All validated live:
        1. Recent errors: `fields @timestamp, @message | filter @message like /<error-token>/ | sort @timestamp desc | limit 20`. Keep `limit 20`: an exception token (`IllegalStateException`, `Caused by`) matches every line of a Java stack trace, so each row is several KB -- a `limit 50` over 30 days returned 129 KB and tripled the context for the rest of the run. For a long window answer HOW OFTEN with query 4 first; to read samples, add `_transform` on `aws_logs_get_query_results` and return only what you need (e.g. `return result.results.slice(0,10).map((r) => r.map((f) => String(f.value).slice(0,300)))`).
        ...
        8. Correlate errors ACROSS several log groups in one query (when a service and its gateway or sidecar log to separate groups; at most 50 groups per query): `SOURCE logGroups(namePrefix: ['<prefix-a>', '<prefix-b>']) | fields @timestamp, @message, @log | filter @message like /(?i)error/ | sort @timestamp desc | limit 100` -- `@log` names the group each row came from.
        9. Lambda cold starts (Lambda groups only; uses the platform REPORT line): `filter @type = "REPORT" | stats avg(@duration) as avg_ms, max(@duration) as max_ms, count(*) as invocations, sum(strcontains(@message, "Init Duration")) as coldStarts by bin(1h)`
        10. Lambda memory pressure (an OOM kill shows as utilization near 100 with a stack trace in the same window): `filter @type = "REPORT" | stats max(@memorySize / 1000 / 1000) as provisioned_mb, max(@maxMemoryUsed / 1000 / 1000) as used_mb, avg(@maxMemoryUsed * 100 / @memorySize) as utilization_pct by bin(1h)`
        11. Lambda timeouts: `filter @message like /Task timed out/ | fields @timestamp, @requestId, @message | sort @timestamp desc | limit 20`
    - **Syntax gotchas (each one is a `MalformedQueryException` that is NOT a window error).** `bin()` units are capped: ms <= 1000, s <= 60, m <= 60, h <= 24, so write `bin(5m)`, never `bin(300s)`. A field name containing a hyphen must be wrapped in backticks (`` `event-name` ``); unquoted it parses as subtraction and returns nonsense with no error. `head`/`tail` are deprecated; use `limit`. Only `filter field = value` and `filter field in [...]` use field indexes; `like` never does, so prefer an equality filter on a known field when the group is large. Never nest subqueries. Query and poll calls are each limited to 10 per second per account; do not fan out more than a handful of `aws_logs_start_query` calls in one iteration.
    - **Report grouping for log findings (six buckets).** When you report errors found in logs, group them into these buckets and give a count per bucket, so the aggregator can compare log evidence across estates and datasources: (1) Application exceptions (unhandled exceptions, stack traces, runtime errors); (2) Connection errors (network timeouts, connection refused, service unavailable); (3) Auth errors (access denied, invalid credentials, permission errors); (4) Resource errors (memory exhaustion, disk, file descriptors); (5) External service errors (third-party or downstream API failures and timeouts); (6) Configuration errors (missing or invalid settings, environment issues). State whether each bucket's errors are steady across the window or arrive in bursts, because a burst that lines up with a deploy, a scaling event, or an alarm transition is the correlation hook. Library query 2 (`pattern`) is the cheapest way to fill the buckets; on Infrequent Access groups fall back to query 4 plus query 6.
    ...
    - **incident window (when the INVESTIGATION FOCUS gives one).** The wide window answers "does this exist, how often". It does NOT answer "what happened at the incident": `sort @timestamp desc | limit N` over 30 days returns the NEWEST N rows, so a run reported a duplicate-key line from two days earlier, and job ids from the day after, as the incident. When the focus block lists an "Incident window", ALSO run the incident query with its relative tokens pasted exactly as given (`startRelative: "now-<n>m"`, `endRelative: "now-<n>m"`); they are computed by the server, so nothing is mis-dated. Attribute a row to the incident only if its `@timestamp` is inside that window, and give the date of any row you cite from outside it.
