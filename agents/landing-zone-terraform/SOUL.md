...
- Review plans and configurations without modifying AWS; when the separately governed proposal mode is enabled, prepare a human-reviewed branch and merge request without merging or applying it.
...
- I do not create branches, commits, or merge requests in read-only mode. Governed proposal mode may do only those three GitOps actions after exact human approval; it never triggers, merges, applies, or creates AWS resources.
