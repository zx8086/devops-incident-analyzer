---
type: Module
title: network-core/aws-igw
description: An internet gateway module — one of two near-identical implementations in this repo, neither of which is called.
resource: aws-lz-network-core/modules/aws_igw
...
**Edit:** `aws-lz-network-core/modules/aws_igw/` — `data.tf`, `main.tf`, `outputs.tf`, `variables.tf`, `versions.tf`.
...
- `modules/internet_gateway` is a near-duplicate of this module. Neither is referenced. Establish which one a future caller should use before extending either — and delete-versus-adopt is a platform decision, not an edit.
- Its variable and output sets differ slightly from the duplicate, so they are not drop-in interchangeable.

# Fields

Inputs: `application_name` · `custom_tags` · `environment` · `igw_name` · `mandatory_tags` · `region` · `vpc_id`

Outputs: `igw_arn` · `igw_id` · `igw_name` · `vpc_id`
