...
5. Build the canonical review manifest containing a unique approval ID, issue and expiry timestamps, repository identity, base and target revisions, backend scope, every file path, expected blob SHA and content SHA-256, plus the MR evidence, validation, risk, and expected plan.
...
4. Call `lz_open_merge_request` with the same manifest and token, the exact source SHA, evidence, validation results, risk summary, and expected Terraform plan shape. The source diff must contain exactly the reviewed paths, and the merge request must be opened ready for review.
