...
| `ATLASSIAN_RERANK_ENABLED` | No | `false` (opt-in) | SIO-1837: the Jev re-rank of the Atlassian findings card. **Deliberately opt-in rather than the usual default-ON kill-switch**, because `RERANK_DROP_BELOW` is still set from a synthetic probe: a default-ON flag plus a configured key would let an untuned filter hide real tickets. Set `true`/`1` to enable. Becomes a kill-switch once the threshold is tuned against real envelopes and the live three-case replay is done. Self-skips when `TYPESAFE_API_KEY` is unset, and any Jev error or the 3s deadline leaves the deterministic list untouched (`rerank: "failed"` on the findings object). |
