...
| `MONITOR_ACTIONABILITY_ENABLED` | No | `true` | SIO-1838: kill-switch for the Jev actionability gate in the pi-coms monitor. Defaults ON, which means the gate JUDGES each warn finding and journals its verdict (`actionability_verdict` rows); it does not act on them. Self-skips when `TYPESAFE_API_KEY` is unset, and any error leaves the investigation batch untouched. Set `false`/`0` to stop judging entirely. |
