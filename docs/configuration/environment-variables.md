...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `KAFKA_TOOL_TIMEOUT_MS` | No | `30000` | **Dual role.** (1) Server-side: per-tool admin RPC timeout in ms, mapped to the `@platformatic/kafka` library option `timeout` (the library's own default is 5 000 ms, which trips on first-call MSK warmup). Provider-supplied timeouts still win — for example, the MSK provider's 60 s override is preserved on top of this value. The pre- config option `requestTimeout` was a no-op (the underlying schema is `additionalProperties: false`); it has been renamed to `timeout` to match the library and is now correctly threaded through. (2) SIO-1115 bridge-side: the agent's `defaultToolTimeout` for `kafka-mcp` (was the 60 s adapter default). Intentionally the same knob so the client's per-call abort tracks the server's own RPC budget and both expire together. |
