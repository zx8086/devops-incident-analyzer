...
- **Correction (post-PR #663 live-verification against the actual app, not just a manual curl probe): the single-package detail endpoint (`GET /api/fleet/epm/packages/{pkgName}`) does NOT accept `withPackagePoliciesCount` as a query param — it returns HTTP 400.** This spec originally documented that shape based on a manual `curl` test that happened to omit the param's interaction with that specific endpoint. The correct call is the **list** endpoint: `GET /api/fleet/epm/packages?withPackagePoliciesCount=true` (no package-name path segment), which returns an `items` array — filter it client-side for the entry whose `name` matches the target integration. Live-verified against 9 of the repo's 10 configured deployments (`eu-onboarding`, `eu-cld`, `ap-cld`, `eu-b2b`, `gl-cld-reporting`, `eu-cld-monitor`, `ap-cld-monitor`, `us-cld-monitor`, `us-cld` — every deployment except `gl-testing`, itself confirmed separately during implementation), spanning three different Elastic Cloud domain suffixes: `.cloud.es.io`, `.elastic-cloud.com`, `.found.io`. **Do not reintroduce the single-package path — it is a real HTTP 400, not a hypothetical.**
- Exact response shape, confirmed live: each matched item's top-level `version` is the package's *latest available* version; `installationInfo.version` is the *currently-installed* version (e.g. live-observed `eu-cld`: installed `2.7.5`, available `2.9.4`); `installationInfo.previous_version` is a bonus field (the version installed before the current one); `status` is `"installed"`/`"not_installed"`; and the policy count is `packagePoliciesInfo.count` (an object, not a bare field).
...
3. Call `GET {kibanaUrl}/api/fleet/epm/packages?withPackagePoliciesCount=true` (the **list** endpoint — no package-name path segment; see the "Correction" note above) with `Authorization: ApiKey {ELASTIC_<DEPLOYMENT>_API_KEY}` and `kbn-xsrf: true` (Kibana requires this header on API calls; omitting it was not an issue in this session's live tests but the header is Kibana's documented convention and should be sent regardless). Parse the response body's `items` array, find the entry whose `name` equals `target.integration`, and read `item.version` (latest/target), `item.installationInfo?.version` (installed, `undefined` if never installed), and `item.packagePoliciesInfo?.count` (affected-policy count) from that matched item — all three field names confirmed live against a real response, not guessed from docs. No match found in `items` degrades the same way a non-2xx response does (best-effort, not an error).
