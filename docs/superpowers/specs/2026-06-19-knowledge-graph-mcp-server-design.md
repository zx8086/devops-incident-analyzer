...
3. **Curated surface ships; raw Cypher is ON by default, read-only.** Four curated `kg_*` tools (one per reader) are always registered — injection-safe, all values bound as params. A `kg_run_cypher` tool is registered **by default** (`KG_MCP_ALLOW_CYPHER=false` to disable); it runs a pure read-only guard that rejects any write/DDL keyword (`CREATE/MERGE/SET/DELETE/DETACH/REMOVE/DROP/ALTER/COPY/CALL/...`) after stripping comments and string literals, rejects multi-statement payloads, and binds `$params`. **Decision (resolved): default on**, so the agent can answer ad-hoc graph questions the curated tools don't cover. To use it safely the agent needs the schema, so the `kg_run_cypher` description embeds a **schema card** (node tables + properties, relationship directions, the two lbug binder quirks, worked examples) mirrored from `schema.ts`, and `agents/elastic-iac/skills/query-knowledge-graph/SKILL.md` gives a fuller version the agent can load on demand. Schema is deliberately NOT stored in agent-memory — it is static, versioned with the code, and belongs in the prompt/tool surface (no recall round-trip, no staleness). Agent memory remains for learned/episodic facts only.
...
**Go.** The prototype is built, wired, tested, and proven end-to-end against the real engine. Decided: `KG_MCP_ALLOW_CYPHER` defaults **on** with a schema card in the tool description + a loadable skill (this revision). Remaining hardening (est. ~0.5 day): (a) optionally extend the same surface to incident-analyzer's incident-side graph; (b) add a readiness gate so the bridge's first connect waits for the in-process `Bun.serve` to bind (today a transient miss self-heals via health-poll reconnect); (c) confirm the AgentCore deployment path mounts the in-process server the same way the local web app does.
