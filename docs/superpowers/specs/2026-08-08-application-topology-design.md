...
## Addendum -- SIO-1460: node-noise reduction (scope, collapse, rank)

Ticket: https://linear.app/siobytes/issue/SIO-1460

The first live run rendered 150 nodes / 185 links (MAX_NODES cap hit): real martech-* services were buried under high-cardinality `dependency` nodes (per-locale storefront hosts, per-queue AMQP destinations, in-process vert.x addresses), and blind first-N truncation could drop the focus services themselves. All fixes are pure and live in `packages/agent/src/application-topology.ts` (with one payload tweak in `app-map-baseline.ts` and one label tweak in `apps/web/src/lib/app-chart.ts`). The build pipeline is now: parse (accumulating host families) -> `collapseHostFamilies` -> `scopeToFocus` -> `capTopology` (ranked). `mergeApplicationTopologyOverlay` is unchanged and runs AFTER scoping, so KG overlay nodes are never scoped away.

- **Bus/in-process handling** (`busDestinationKind`): kafka and `vert.x/*` are skipped entirely (unchanged kafka rationale; vert.x is an in-process event bus, never a network dependency). `AMQP 1.0/*` destinations collapse to ONE `dep:amqp-bus` node named "AMQP broker" -- RabbitMQ has no consumer-group system of record, so a single broker node keeps its presence on the map without the per-queue storm. Tradeoff: AMQP producer topology is not modeled per-queue (a broker-node model is a future ticket).
- **Host-family collapse** (`parseHost` + `hostFamily` + `collapseHostFamilies`): host-shaped destinations (hostname[:port], non-IP, ≥1 dot) group by their **brand label** -- the label before the public suffix, TLD-agnostic -- so a fleet of per-locale ccTLD storefronts (`www.calvinklein.de`, `.es`, `.co.uk`, ...) collapses to ONE node. `hostFamily(host)` returns `{ brand, domain }`: brand is the family key; domain (approximate registrable domain: last two labels, three for a two-part public suffix like `co.uk`) drives the name. Naming: one host -> raw endpoint; many hosts of one registrable domain -> `"<domain> (N hosts)"`; many hosts spanning multiple TLDs of one brand -> `"<brand> (N locales)"`. Distinct brand labels never merge (`calvinkleinservice` stays separate from `calvinklein`). Edge targets point at `dep:<brand>` at creation time (cross-source dedup collapses to one edge per source). Display-only: `dep:` ids never reach the KG (`application-topology-kg.ts` strips only `svc:`/`cg:`/`topic:`) and the overlay never mints `dep:` ids. **Junk filter:** a non-host resource with no letters and no dot (`"0"`, `"-"` -- APM placeholders for uninstrumented exit spans, observed live) is dropped rather than rendered as a node; IP literals (which have dots) still render.
- **Focus-scope retention** (`scopeToFocus`): on focused turns (`focusServices.length > 0`) keep anchor nodes (ANY node with `service` set -- `svc:` AND focus-matching `cg:`) plus their 1-hop neighborhood; drop the rest. No-op on unfocused turns (`matchesFocus([])` is show-all). An empty anchor set falls back to show-all so scoping never blanks a card with real data (the SIO-1030 empty-collapse contract).
- **Ranked truncation** (`capTopology`): sort before slicing -- focus anchor (`service` set) > `kind:"service"` > degree desc (from `acc.edges`) > insertion order (stable). Shared with the overlay cap path (harmless; overlay sets are small). The `MAX_EDGES` slice stays insertion-order (fires only after nodes are bounded).
- **Baseline shrink** (`app-map-baseline.ts`): `MAX_SOURCE_SERVICES` 100->50, `MAX_DESTINATIONS_PER_SERVICE` 50->25 -- pure payload/latency saving (the card/prompt/KG use far less; KG caps at 50 edges).
- **Dependency de-labeling** (`app-chart.ts`): `dependency` removed from `LABELED_KINDS` -- names show on hover/emphasis and in the SIO-1459 text view, cutting the label storm.

New tests in `application-topology.test.ts` (bus collapse, host-family collapse incl. `co.uk` and single-host, scope 1-hop incl. cg-anchor and empty-match fallback, ranked cap incl. high-degree hub and last-inserted anchor, scoped prompt summary), `app-map-baseline.test.ts` (shrunk `terms.size`), `app-chart.test.ts` (dependency unlabeled).
