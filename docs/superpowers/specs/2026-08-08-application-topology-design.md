...
- **Host-family collapse** (`parseHost` + `registrableDomain` + `collapseHostFamilies`): host-shaped destinations (hostname[:port], non-IP, ≥1 dot) group by an approximate registrable domain (last two labels; three for a two-part public suffix like `co.uk`). The family node is named `"<family> (N hosts)"` when N > 1, else the sole raw host. Edge targets point at `dep:<family>` at creation time (so cross-source dedup collapses to one edge per source). Display-only: `dep:` ids never reach the KG (`application-topology-kg.ts` strips only `svc:`/`cg:`/`topic:`) and the overlay never mints `dep:` ids.
