...
- **Built against the README-implied convention** (`environments/<dep>/<stack>/<for_each-key>.json`, provider attr = top-level JSON key), overridable via `ELASTIC_IAC_STACK_CONFIG_TEMPLATE`. **To finalize:** confirm the real `agent-policies` layout with the elastic-iac team (is the filename the `for_each` key? are attrs top-level or nested?) and adjust the template/projection if it differs. Reconcile *direction* (to-live vs to-declared) remains the operator's runtime call (handover section 5).
- **SIO-890 (follow-up):** report-sourced reconcile is now **default-on for every stack** that is not a deployment/ilm family — the opt-in `ELASTIC_IAC_CONFIG_REPORT_STACKS` allowlist is removed. Opt out per stack via `ELASTIC_IAC_REPORT_STACKS_EXCLUDE` (default none -> whole stack). Safe because reconcile-to-live is only offered when the drift has writable values, and `buildReportSourcedReconcile` fails safe (blocks on an unreadable/mis-resolved file) so a non-conforming stack never writes garbage.
