...
### incident-closure learning chain (SIO-1357)

The second production SkillsFlow, and structurally different from the resolve-identifiers presets: it runs **post-turn, in the background**, not inside the graph. `agents/incident-analyzer/workflows/incident-close.yaml` is a three-step chain — `postmortem` (skill `incident-postmortem`, reconstruct a blameless postmortem from the closing turn's aggregated `report`/`findings`/`confidence`) → `post_wiki` (skill `wiki-ingest`, compile it into a wiki-page-update proposal) → `learn` (open a human-review PR for the durable learning). It is `trigger: manual` and fired only when `detectCloseCommand()` (`packages/agent/src/close-detect.ts`, matching the chat command `close incident`) sees the command; the trigger payload carries the already-completed investigation (`${{ trigger.report }}`, `${{ trigger.confidence }}`), because the workflow runs **after** the SSE/interrupt turn has already answered. Wiring lives in `packages/agent/src/close-workflow.ts` + `incident-close-workflow-handlers.ts`. Best-effort throughout (`error_handling` tolerant): a failure anywhere in the chain must never affect the user-facing turn that already resolved the incident. Contrast with the resolve-identifiers presets, which run **synchronously inside** the `resolveIdentifiers` graph node before fan-out.
