...
| 2c: third graph (LLM spoke selection and synthesis) | [SIO-1655](https://linear.app/siobytes/issue/SIO-1655) | Deferred, NOT approved; plan `docs/superpowers/plans/2026-09-06-pi-fleet-phase-2c.md`; two blockers to decide first (in-process persona constraint, spoke replies as model input) |
...
No phase starts before its issue is approved. Phase 2 decision recorded 2026-09-06: the thin hub pane (2a) first, the fleet inbox node (2b) added the same day; the third graph (2c) stays a later option. 2c was issued and planned on 2026-09-06 (SIO-1655) so it is tracked and costed, but it remains deferred and unapproved: its trigger condition (an LLM must choose which spokes to ask and synthesize replies) has not occurred, and it carries two decisions that must be recorded before any code -- whether the `pi-fleet` persona may run in-process at all (SIO-1649 says it must not), and how spoke replies may enter a model prompt without breaking the PR #682 invariant that hub replies are data, never an LLM input.
