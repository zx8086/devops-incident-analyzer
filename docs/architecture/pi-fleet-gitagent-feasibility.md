...
- **Release layer (the unused gitagent capabilities)**: make `agent.yaml` `version` load-bearing, tag releases `pi-fleet-vX.Y.Z`, and have CI validate and export a Pi package (AGENTS.override.md + skills + package.json with a `pi` manifest). The fleet publish step runs the same exporter and vendors the result into the S3 bundle; with one codebase (decision below) there is no release asset between repos.
...
## Decision: one codebase (user, 2026-09-06)
...
- `.github/workflows/agent-release.yml` on `push: tags: ["pi-fleet-v*"]`: typecheck, bridge tests (skill spec and OKF audit included), export, version-equals-tag gate. No release upload: the fleet publish step invokes the same exporter.
- `docs/architecture/gitagent-bridge.md`: release section.

pi-coms:
- `packages/pi-coms/deploy/publish-fleet.sh` runs the exporter and vendors the result; `packages/pi-coms/deploy/bootstrap/agent-bootstrap.sh` copies `vendor/pi-fleet/aws-spoke/AGENTS.override.md` (replacing the current `deploy/AGENTS-spoke.md` copy), installs skills to `$AGENT_HOME/.pi/agent/skills/`, appends `persona=pi-fleet-vX.Y.Z` to `--purpose`; retire `AGENTS-spoke.md` and the package `AGENTS.md` in favour of exported files, with `just sync-persona` for laptop operators.
...
| Fleet bundle built from a workspace subtree resolves dependencies differently | Medium | Stage the subtree with its own lockfile; smoke a dev host before any prd publish |
| Operators' git-URL Pi installs go stale after the move | Certain | One-time reinstall from the local checkout path; old repo README points here |
...
| 2c: third graph (LLM spoke selection and synthesis) | [SIO-1655](https://linear.app/siobytes/issue/SIO-1655) | Done (PR #697 registry, squash `756716e2`; PR #698 graph, squash `9f125775`); separate in-process persona `agents/pi-fleet-console/`; capability ON by default since PR #699 (`74b795bc`), offered only where a hub is configured; live run with registered spokes user-run |
...
No phase starts before its issue is approved. Phase 2 decision recorded 2026-09-06: the thin hub pane (2a) first, the fleet inbox node (2b) added the same day; the third graph (2c) stays a later option. 2c was issued, planned and built on 2026-09-06 (SIO-1655) at the user's instruction. Both open questions were decided first and are recorded on the issue: it uses a SEPARATE in-process persona (`agents/pi-fleet-console/`), leaving `agents/pi-fleet/` export-only and the SIO-1649 constraint intact; and spoke replies enter the model only inside the console graph, wrapped as untrusted evidence by `wrapUntrusted` -- the one deliberate, documented exception to the PR #682 invariant. It shipped as two PRs: the registry refactor alone (#697, behaviour-preserving), then the graph (#698). Gated off by `PI_FLEET_GRAPH_ENABLED`, so the default deployment still offers exactly the two prior agents.
