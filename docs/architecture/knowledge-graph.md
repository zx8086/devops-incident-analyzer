...
- **Rebuild:** `knowledge-graph:rebuild` replays the Couchbase Agent Memory mirror facts back into a fresh graph (the "graph is a rebuildable projection" story). It replays `kg-incident` (Incident + `AFFECTED_BY`), `kg-root-cause` (RootCause + `HAS_ROOT_CAUSE`), and `kg-binding` (telemetry bindings) in dependency order. It also replays each `kg-lz-gitlab-import-start` anchor into a minimal Landing Zone repository plus a checkpoint at the original controlled backfill timestamp. The next GitLab reconciliation therefore rehydrates history into the fresh graph rather than restoring the latest watermark and skipping missing records. Still NOT rebuildable: Incident **embeddings** (facts carry no vector; re-embed is a deliberate Bedrock cost, not default) and `Finding`/`CORRELATES_WITH` (graph-only). The CLI prints exactly what it could not rebuild.

### The network map (SIO-1204, per-incident)

A per-incident network subgraph written by `recordBindings` (behind `KG_NETWORK_WRITE_ENABLED`, default ON, inert without `KNOWLEDGE_GRAPH_ENABLED`). The pure builder (`packages/agent/src/network-topology.ts`) parses the turn's toolOutputs (EC2/ECS/ELBv2/Route53 plus Kong/Kafka/Capella/Elastic endpoints) into the per-turn `state.networkTopology` (ECharts card + `network_topology` SSE event), and `deriveNetworkTopology` (`packages/agent/src/network-kg.ts`) maps it onto the KG boundary record for `recordNetworkTopology`/`recordIpBinding` (`packages/knowledge-graph/src/writer.ts`).

Nodes: `Vpc`, `Subnet` (cidr), `LoadBalancer`, `TargetGroup`, `DnsRecord` (PK `<name>:<type>`), `IpAddress` (PK is the bare ip string; in reality private IPs are only unique per VPC, so the same node can legitimately carry edges from several VPCs -- readers return ALL currently-valid hits with subnet/VPC context for live disambiguation), `Endpoint` (PK `<host>:<port>`). Edges (all born with lifecycle columns): `IN_VPC`, `IN_SUBNET` (CIDR containment computed in TypeScript at ingest -- lbug has no UDFs, unlike the Neo4j `ipBelongsToNetwork` reference), `ATTACHED_TO`, `HAS_TARGET_GROUP`, `FORWARDS_TO`/`FORWARDS_TO_IP`, `RESOLVES_TO_LB`, `HAS_ENDPOINT` (Service anchor), and the bi-temporal `BOUND_TO` (IpAddress -> AwsResource): one valid owner at a time, re-binding invalidates the prior edge (`tInvalid` set, never deleted -- as-of reads keep the history). Deliberately NOT in `TOPOLOGY_KINDS`/the cron sweep: per-incident collection is partial, and sweeping a partial collection would accrue false misses (a future network sweep collector is a follow-up ticket).
...
### The application map (SIO-1457, per-incident)

The service-to-service sibling of the network map, riding the same `recordBindings` write (behind `KG_APP_MAP_WRITE_ENABLED`, default ON, inert without `KNOWLEDGE_GRAPH_ENABLED`). The pure builder (`packages/agent/src/application-topology.ts`) parses the turn's elastic APM destination aggregations and kafka consumer-group outputs into `state.applicationTopology` (ECharts `ApplicationTopologyCard` + `application_topology` SSE event); `deriveApplicationTopology` (`packages/agent/src/application-topology-kg.ts`) maps observed edges onto `TopologyEdgeRecord`s for `recordAppMapTopologyEdges` (`packages/knowledge-graph/src/writer.ts`).

Writes only `DEPENDS_ON` (svc->svc `calls`; `dependency` nodes like postgresql never become Service rows) and `CONSUMES_FROM` (cg->topic), capped 50/kind, stamped `discoveredBy: "app-map"`. **Third writer** on these rel tables (after the topology sweep and Orbit), reusing the Orbit never-demote collision policy: a currently-valid `topology-job`-owned edge is untouched, and only unowned or `app-map`-owned edges are claimed. Deliberately NOT in `TOPOLOGY_KINDS`/the cron sweep (per-incident collection is partial; miss-counting it would invalidate real edges).

Read path: `appMapForServices(store, services, asOf?)` (`reader.ts`) returns currently-valid `DEPENDS_ON`/`RUNS_ON`/`ROUTES_TO` edges per focus service plus name-affinity-filtered `CONSUMES_FROM`, with `discoveredBy` for attribution. `graphEnrich` converts rows into `applicationTopologyOverlay` state edges (shared id-prefix contract `svc:`/`topic:`/`cg:`/`aws:`/`route:`), which `extractFindings` merges into the built map as dashed `priorKnowledge` edges -- overlay-only turns still render, so the card shows what the KG already knows even when this turn's tools produced nothing.

### The scheduled topology sweep (SIO-1104, 5a; SIO-1358)

A **third write path**, belonging to neither agent pipeline: a schedule (`schedules/kg-topology-sweep.yaml`) declaratively registered via the generic scheduler in `packages/skillflow/src/scheduler.ts`, running a one-step `workflows/kg-topology-sweep.yaml` whose `node:` target is bound to `runTopologySweep` in `packages/agent/src/kg-topology.ts` -> the KG writers, sharing the same MCP bridge and the single `getGraphStore()` lbug handle. Requires `KNOWLEDGE_GRAPH_ENABLED` (a backend precondition); cadence and on/off (`enabled:`) both live in the schedule's YAML, `cron: "0 * * * *"` (hourly) but shipping **`enabled: false`** (opt-in per deployment) -- no `KG_TOPOLOGY_CRON_ENABLED`/`KG_TOPOLOGY_CRON_SCHEDULE` env vars (removed by SIO-1358; previously a hand-wired `apps/web/src/lib/server/kg-topology-cron.ts` Bun.cron file, now deleted).
...
So `startKnowledgeGraphServer()` mounts the server on `127.0.0.1:9087` **inside the web app** (`apps/web/src/lib/server/knowledge-graph-server.ts`, called from `agent.ts`) -- `Bun.serve` under Bun, `node:http` under Vite SSR where the `Bun` global does not exist (SIO-986) -- and its tools reuse the same-process `getGraphStore()` singleton. The mount is gated on `KNOWLEDGE_GRAPH_ENABLED`, fire-and-forget, and **best-effort** — a start failure only disables the `kg_*` tools, never blocks the app.

**Surviving a Vite in-place restart (SIO-1645).** Vite restarts its dev server in place on any root `.env*` change: same PID, a new SSR module runner, `import.meta.hot.dispose` NOT run, and linked workspace packages re-evaluated. Two process-lifetime singletons therefore live on `globalThis` slots (`Symbol.for`, the SIO-1113 health-poll / SIO-1468 scheduler idiom) rather than module scope: the started server (`knowledge-graph-server.ts`) and the lbug store (`packages/knowledge-graph/src/store.ts`, `getGraphStore()`). A re-evaluated module graph reuses both, so the process never opens a second `lbug.Database` on the same path (a same-process second open succeeds but the two handles do not share buffers or uncommitted writes, and `close()` is a no-op per SIO-954). The surviving listener keeps dispatching into the module graph that created it, so edits to the KG server's own tool code need a full dev-server process restart; its data is consistent because the store is shared.

**Pre-flight (SIO-987, SIO-1645).** When the port is already in use and this process holds no slot, the mount asks the listener's own `GET /identity` and classifies it (`classifyPortOccupant` in `packages/mcp-server-knowledge-graph/src/preflight.ts`): this process's own server (reuse, info); another process on the **same** store (the lock-out warning -- the agent's in-process writes will fail -- with read-only `kg_*` tools registered against it); another process on a **different** store (warning, tools NOT registered: they would read a different graph); a non-KG service (warning, tools not registered); unidentifiable (legacy warning, tools registered). Binding is never attempted against a live listener (that produced a misleading `EADDRINUSE` + "Fatal" log).
...
- **incident-analyzer:** `grep -c addNode packages/agent/src/graph.ts` = **31** — 21 base nodes + the 4 gated KG nodes (`recordEntities`, `graphEnrich`, `recordRootCause`, and SIO-1100 `recordBindings`) + the 6 gated HIL-learning nodes (`learnFetchTicket`, `learnMatchIncident`, `learnMatchGate`, `learnDistill`, `learnReviewGate`, `applyLearnings`, SIO-1126). The HIL nodes are separate from the four KG pipeline nodes above — they gate on `HIL_LEARNING_ENABLED` (default on), not `KNOWLEDGE_GRAPH_ENABLED`, and branch off `classify` on an explicit `learn from TICKET-123` command (see [agent-pipeline.md](agent-pipeline.md#hil-learning-lane)). They still use the graph (the lane requires `KNOWLEDGE_GRAPH_ENABLED`, matches a KG `Incident`, and curates it); they are listed here so the total reconciles with the grep.
