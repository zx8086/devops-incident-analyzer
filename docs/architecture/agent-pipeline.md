...
The agent pipeline is a LangGraph StateGraph that processes user queries through classification, normalization, optional runbook selection, entity extraction, optional AWS estate expansion, parallel datasource querying, cross-datasource alignment, aggregation, typed-findings extraction, mandatory cross-agent correlation enforcement, confidence gating, validation, branched mitigation proposal (investigate / monitor / escalate), follow-up generation, and topic-shift detection. The graph is defined in `packages/agent/src/graph.ts` and compiled with a checkpointer for conversation persistence. Node count: **20 base nodes, 23 with the knowledge graph enabled** — the three gated KG nodes (`recordEntities`, `graphEnrich`, `recordRootCause`) are always registered but edged only when `KNOWLEDGE_GRAPH_ENABLED` is set (the SIO-640 edge-gate idiom), so `grep -c addNode packages/agent/src/graph.ts` = 23. See [knowledge-graph.md](knowledge-graph.md) for those nodes.
...
** wall-clock budgets.** Both budgets are env-tunable so an operator can extend them on slow links without code changes:
- `GRAPH_TIMEOUT_MS` overrides the gitagent manifest `runtime.timeout` (default raised from 600 s to 720 s) — the graph-level abort signal.
...
See [Environment Variables > Agent Configuration](../configuration/environment-variables.md#agent-configuration) for both knobs.
...
|------------|----------------|-------------------|---------------------|
| elastic | ~93 (77 cluster + 16 cloud/billing on `EC_API_KEY`) | 11 (search, cluster_health, node_info, index_management, shard_analysis, ingest_pipeline, template_management, alias_management, document_ops, snapshot, diagnostics) plus cloud/billing (deployment_audit, plan_history, hardware_profile_simulation, cost_attribution) | 3-15 |
