...
**Confidence cap mechanism (SIO-681 / SIO-707 / SIO-709 / SIO-712):** The aggregator caps `confidenceScore` at `0.59` (strictly below the 0.6 HITL gate) whenever any of these inputs fire — independent of which one:
...
2. **Tool-error rate >= 15%** (SIO-707 / SIO-709). When the sub-agents' aggregated tool-call failure rate crosses 15% (originally 25%, tightened by SIO-709). The styles-v3 transcript at kafka 9/40 (22.5%) and elastic 4/27 (14.8%) was the motivating signal — both now cap, where neither did under the 25% bar.
3. **Gaps section >= 2 bullets** (SIO-709). The aggregator output's `## Gaps` section (exact heading, pinned by SIO-711) parses for bullets. >= 2 entries caps confidence regardless of `degradedRules`.
4. **`skipCoverageCheck` rule degraded** (SIO-712). The contradiction banner described above also caps.
...
**Defensive-prose ban (SIO-711):** The aggregator system prompt forbids self-defending language like "not fabricated", "all data is real", and similar. The model must instead surface uncertainty via `[partial: <field>]` markers inline and structured entries under the `## Gaps` heading. The exact `## Gaps` heading is load-bearing for the SIO-709 parser; alternate forms (`## Known Gaps`, `### Gaps`) will not be detected.
...
**SIO-697 retry narrowing.** Earlier alignment retries re-ran every Elastic deployment, including ones that succeeded on the first pass — doubling wall-clock cost for a single missing deployment. The `align` node now writes `retryDeployments` (state field) containing only the failed deployment IDs; `queryDataSource` reads it on the retry path (`isRetry && retryDeployments.length > 0`) and fans out only over that subset. On the same change, the Elastic deployment dispatcher switched from sequential `for-await` to `Promise.all`; `withElasticDeployment` uses `AsyncLocalStorage` so each parallel branch sees its own deployment context.

**SIO-697 wall-clock budgets.** Both budgets are env-tunable so an operator can extend them on slow links without code changes:
