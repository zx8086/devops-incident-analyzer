...
| `confidenceCap` | `number \| undefined` | Replace | Set to `0.59` by `enforceCorrelations` when one or more correlation rules are degraded; the aggregator then writes `confidenceScore = min(currentScore, confidenceCap)`. The 0.59 value is intentionally strictly below the 0.6 HITL gate (`checkConfidence` uses `<`), so a capped run cannot accidentally clear the gate. `undefined` on the satisfied / no-rules path. |
...
|------------|----------------|-------------------|---------------------|
| elastic | ~102 (86 cluster incl. 9 ML anomaly-detection + 16 cloud/billing on `EC_API_KEY`) | 12 (search, cluster_health, node_info, index_management, shard_analysis, ingest_pipeline, template_management, alias_management, document_ops, snapshot, diagnostics, ml_monitoring) plus cloud/billing (deployment_audit, plan_history, hardware_profile_simulation, cost_attribution) | 3-15 |
