...
## PR #730 detail (SIO-1695, the five High prompt-audit findings)

Eight files: three persona prompts, two tool YAMLs, one manifest, `llm.ts` and `model-factory.ts`. The change set is the High subset of the 2026-09-11 prompt audit (`experiments/PROMPT-AUDIT-2026-09-11.md`, kept local): the root SOUL's 1-hour default window aligned to the 24 hours the rest of the pipeline has used since SIO-1296; the Elasticsearch and Atlassian `action_descriptions` cut back to the one-sentence contract because that field reaches only the entity extractor, with the steering moved into the SOULs of the sub-agents that actually run the tools; the pi-fleet-console manifest given the model block it never had (`model-factory.ts` used to fall back to `claude-sonnet-4-6` silently, and now throws); and Bedrock's cache counters added to the per-call usage log so the SIO-1040 cache point is measurable for the first time.

**Greptile:** SKIPPED at dispatch (16:28Z, 152 ms) and again on a manual re-trigger through the MCP server (23:03Z, 113 ms). Forty-seventh consecutive skip; #724 through #729 (2026-09-10 to 2026-09-11) were all skipped as well and have no detail section here.

**CodeRabbit:** nothing in the six and a half hours the PR was open. Forty-seventh consecutive absence.

**Local review (substitute):** the audit itself ran four parallel reviewers over the prompt surface with per-finding provenance (`git log -L`) before any patch was written, and the three High claims that drive the code changes were re-verified by hand: `action_descriptions` has exactly one consumer (`entity-extractor.ts:72`); no file reads `cacheReadInputTokens`; the console manifest has no `model:` key. The patches were generated against scratch copies and checked with `git apply --check` before touching the tree.
