...
**1. Per-server feature gates (Kafka).** Boolean env vars enable groups of write tools at runtime. After, `KAFKA_ALLOW_WRITES` and `KAFKA_ALLOW_DESTRUCTIVE` apply across kafka-core, Connect, Schema Registry, and REST Proxy in a unified way.
...
**2. Shared bootstrap chokepoint (Elastic, Konnect —).** Read-only enforcement lives at the MCP dispatcher level, not on individual tools. When a server calls `createMcpApplication({ readOnly })` from `@devops-agent/shared`, `installReadOnlyChokepoint()` (`packages/shared/src/read-only-chokepoint.ts:43-76`) intercepts the `tools/call` request and consults a `ReadOnlyManagerLike` adapter before invoking the underlying tool handler. The manager owns the classification (e.g., Elastic's `OperationType` set in `packages/mcp-server-elastic/src/utils/readOnlyMode.ts:12-19`); tools do not opt in individually. The legacy per-tool `withReadOnlyCheck()` wrapper was removed from every Elastic tool in commit `08d4cef` — do not add new wrappers in tool files.
...
When `EC_API_KEY` is set, the server additionally registers 16 org-scoped tools that talk to `https://api.elastic-cloud.com` (10 cloud + 6 billing). The original 7 were extended by ��826 with `get_account`, `get_es_resource`, `cancel_pending_plan`, `list_hardware_profiles`, `get_hardware_profile`, `simulate_hardware_profile_change` (with `rate_source_confidence`), `list_instances`, `get_instance_items`, `get_instance_charts`. These do not accept the `deployment` arg and use `EC_DEFAULT_ORG_ID` as the org-id fallback for billing tools. See [Environment Variables](../configuration/environment-variables.md#elastic-cloud-deployment--billing-api-sio-674) and [MCP Server Configuration](../configuration/mcp-server-configuration.md#elastic-cloud-deployment--billing-tools-sio-674-sio-822826) for the full tool list.
...
AWS tools are estate-scoped. Every tool's Zod schema includes a top-level `estate` field; the registration pipeline in `packages/mcp-server-aws/src/index.ts` wraps each handler via `aws-tool-estate-wrapper.ts`, which resolves the estate ID to a cross-account `AssumeRole` session before invoking the underlying AWS SDK v3 client. When adding a new AWS tool:

1. Place the operation file under `packages/mcp-server-aws/src/tools/<service>/` (e.g. `cloudwatch/`, `ec2/`).
2. The Zod schema must declare the `estate` field via `estate-schema.ts` (provides the shared definition).
3. The handler receives a `clientFactory` from `services/client-factory.ts` that already has the per-estate credentials applied — never call `new EC2Client(...)` directly.
4. Register the tool in the service's `index.ts` `register()` function, which the central `tools/register.ts` aggregates.
5. Use `estate-validator.ts` to enforce that the requested estate exists in `AWS_ESTATES` at parse time, not after the SDK call.

Tools added this way automatically appear in `aws_list_estates`'s per-estate capability matrix. Multi-estate also means **tool counts grow linearly with estates at the agent layer** — the `awsEstateRouter` fans out one Send per estate when AWS is in the target datasources, so a single user query can dispatch the same tool N times in parallel.

### Proxy-pattern servers (GitLab, Atlassian)

GitLab and Atlassian MCP servers don't follow the standard "implement every tool locally" pattern. They proxy tool discovery from an upstream MCP endpoint (`{instanceUrl}/api/v4/mcp` for GitLab, the Rovo Atlassian Cloud MCP for Atlassian) at startup, then re-register the discovered tools with a `gitlab_` / `atlassian_` prefix. **Custom local tools** sit alongside the proxy surface — these follow the standard pattern in this guide. See [MCP Server Configuration](../configuration/mcp-server-configuration.md#hybrid-tool-architecture) and [GitLab MCP Server](../configuration/mcp-server-configuration.md#gitlab-mcp-server) for the architecture.
