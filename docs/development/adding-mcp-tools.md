...
When `EC_API_KEY` is set, the server additionally registers 16 org-scoped tools that talk to `https://api.elastic-cloud.com` (10 cloud + 6 billing). The original 7 were extended by SIO-822–826 with `get_account`, `get_es_resource`, `cancel_pending_plan`, `list_hardware_profiles`, `get_hardware_profile`, `simulate_hardware_profile_change` (with `rate_source_confidence`), `list_instances`, `get_instance_items`, `get_instance_charts`. These do not accept the `deployment` arg and use `EC_DEFAULT_ORG_ID` as the org-id fallback for billing tools. See [Environment Variables](../configuration/environment-variables.md#elastic-cloud-deployment--billing-api) and [MCP Server Configuration](../configuration/mcp-server-configuration.md#elastic-cloud-deployment--billing-tools-sio-674-sio-822826) for the full tool list.
