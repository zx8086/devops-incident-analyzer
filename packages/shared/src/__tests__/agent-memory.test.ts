...
import { type AgentMemoryConfig, createFetchAgentMemoryClient, resolveAgentMemoryConfig } from "../agent-memory.ts";
...
describe("createFetchAgentMemoryClient", () => {
	...
	test("addMessages sends a TTL but addFacts sends null TTL", async () => {
		const { calls, restore } = stubFetch({
			"POST /users/incident-analyzer/sessions/t-1/memory": { status: 200, body: { block_ids: ["b1"] } },
		});
		const client = createFetchAgentMemoryClient(CONFIG);
		await client.addMessages(REF, [{ user_content: "q", assistant_content: "a" }], 3600);
		await client.addFacts(REF, ["a durable fact"]);
		expect(calls[0]?.body).toMatchObject({ messages: [{ user_content: "q" }], memory_block_ttl: 3600 });
		expect(calls[1]?.body).toMatchObject({ facts: ["a durable fact"], memory_block_ttl: null });
		restore();
	});

	test("search filters out non-ready blocks and returns summary ?? fact", async () => {
		const { calls, restore } = stubFetch({
			"POST /users/incident-analyzer/sessions/t-1/memory/search": {
				...
				body: {
					count: 3,
					memory_blocks: [
						{ status: "ready", summary: "ready summary" },
						{ status: "processing", summary: "should be hidden" },
						{ status: "ready", fact: "ready fact" },
					],
				},
			},
		});
		const client = createFetchAgentMemoryClient(CONFIG);
		const hits = await client.searchMemory(REF, "kafka lag", { allSessions: true, relevantK: 5 });
		expect(hits).toEqual(["ready summary", "ready fact"]);
