...
function readRows(): CountRow[] {
	...
	const rows = db
		.query<CountRow, []>("SELECT server, tool, calls, failures FROM mcp_tool_call_counts ORDER BY server, tool")
		.all();
	db.close(false);
	return rows;
}
...
describe("agentcore-proxy tool-call metrics", () => {
	test("a successful proxied tools/call counts one ok call", async () => {
		...
		expect(readRows()).toEqual([{ server: "aws-mcp-server", tool: "aws_call_aws", calls: 1, failures: 0 }]);
	});

	test("an inner isError result counts one failure", async () => {
		seedResponses(sseInnerError(2, "MCP error -32603: throttled"));

		const response = await callProxy(2, "aws_run_script");
		expect(response.status).toBe(200);

		expect(readRows()).toEqual([{ server: "aws-mcp-server", tool: "aws_run_script", calls: 1, failures: 1 }]);
	});
	...
	test("a retried call counts once (retries are not separate calls)", async () => {
		...
		expect(readRows()).toEqual([{ server: "aws-mcp-server", tool: "aws_get_tasks", calls: 1, failures: 0 }]);
	});

	test("a terminal TCP failure counts one failure", async () => {
		...
		expect(readRows()).toEqual([{ server: "aws-mcp-server", tool: "aws_list_regions", calls: 1, failures: 1 }]);
	});
