// src/v2/prompts.ts
//
// SIO-1443: v2 port of the generate_sqlpp_query prompt
// (packages/mcp-server-couchbase/src/prompts/sqlppQueryGenerator.ts). Re-implemented against
// @modelcontextprotocol/server's registerPrompt. v1 already uses the raw-shape argsSchema form
// (a plain object of Zod fields, not wrapped in z.object()), which is also v2's shape for the
// Args extends ZodRawShape overload, so the argsSchema and callback body are ported verbatim --
// only the McpServer import source changes.
//
// Prompts are NOT chokepoint/logging-wrapped in v1 (server.ts's registerSqlppQueryGenerator call
// sits outside the tool-wrapping composition, same as resources), so this file takes only
// `server` -- no `tools` Map.
...
7. Provide a brief explanation of how the query works`,
	...
	logger.info("v2 prompts registered successfully");
}
