...
if (import.meta.main) {
	createMcpApplication<AtlassianDatasource>({
		...
		initDatasource: async () => {
			...
			const siteUrl = config.atlassian.siteName ? `https://${config.atlassian.siteName}.atlassian.net` : undefined;
		...
		createTransport: (serverFactory, ds, identityCard) => {
			...
			return createTransport(ds.config.transport, serverFactory!, atlassianProbe, identityCard);
		},
