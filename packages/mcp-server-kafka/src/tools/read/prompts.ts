...
export const LIST_DLQ_TOPICS_DESCRIPTION = `[READ] List dead-letter queue topics (matched by naming conventions -dlq, dlt-*, *-dead-letter, dead-letter-*, *.dlq, DLQ_*) with totalMessages and recentDelta. Returns { topics, matched, sampleFailed, note? }: matched counts DLQ-named topics; sampleFailed counts those omitted because offset sampling failed (they still exist). By default samples offsets twice ~10s apart so recentDelta = secondSample - firstSample reveals live inflow vs. historical accumulation; above 15 matched topics the delta window is auto-skipped (recentDelta:null). Pass skipDelta:true for a fast single-sample probe. Use to detect DLQs growing right now, which often indicates a stalled consumer or repeated processing failures.`;
