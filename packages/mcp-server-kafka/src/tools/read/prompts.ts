...
export const CONSUME_MESSAGES_DESCRIPTION = `[READ] Read messages from a Kafka topic. Creates an ephemeral consumer that does not affect existing consumer groups. IMPORTANT: by default the consumer starts at the LATEST offset and only sees messages produced during the timeout window -- existing backlog is invisible. Pass fromBeginning:true to read historical messages, or use kafka_get_message_by_offset for a specific offset. Pass timestamp (Unix ms) to start each partition at the first offset at or after that time instead of scanning from earliest/latest -- this is the correct choice when searching for an event whose approximate time is known (e.g. an incident timestamp), rather than a full fromBeginning scan of a large topic. fromBeginning takes precedence if both are set. An empty result returns { messages: [], mode, note } explaining the likely cause. Messages whose value is not valid UTF-8 text (Avro/Protobuf) are flagged valueLooksBinary:true.`;
