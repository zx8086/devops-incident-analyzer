...
export const ProduceMessageParams = z.object({
	...
	messages: z
		.array(
			z.object({
				...
				headers: z.record(z.string(), z.string()).optional().describe("Message headers as key-value pairs"),
	...
	acks: z.number().int().min(-1).max(1).optional().describe("Acknowledgment level: -1 (all), 0 (none), 1 (leader)"),
});

export const CreateTopicParams = z.object({
	name: z.string().min(1).describe("Topic name to create"),
	partitions: z.number().int().min(1).max(1000).optional().describe("Number of partitions (default: 1)"),
