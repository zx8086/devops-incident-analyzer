...
export class SchemaRegistryService {
	...
	async getSchema(subject: string, version: number | "latest" = "latest"): Promise<SchemaInfo> {
		return this.request<SchemaInfo>("GET", `/subjects/${encodeURIComponent(subject)}/versions/${version}`);
	}
	...
	async registerSchema(
		...
		return this.request<{ id: number }>("POST", `/subjects/${encodeURIComponent(subject)}/versions`, {
			schema,
			schemaType,
		});
	}
	...
	async setSubjectConfig(compatibilityLevel: string, subject?: string): Promise<{ compatibility: string }> {
		const path = subject ? `/config/${encodeURIComponent(subject)}` : "/config";
