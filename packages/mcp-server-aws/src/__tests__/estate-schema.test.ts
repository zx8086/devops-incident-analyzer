...
import { estateField, withEstate } from "../tools/estate-schema.ts";
...
describe("estate-schema (SIO-853)", () => {
	...
	test("estate field is a string, not an enum", () => {
		const field = estateField(config);
		// ZodString and ZodEnum are disjoint, so the positive check alone proves it is
		// not an enum; no separate not.toBeInstanceOf(ZodEnum) assertion needed.
	...
	test("rejects empty string", () => {
		expect(estateField(config).safeParse("").success).toBe(false);
	});
	...
	test("throws when no estates configured", () => {
		expect(() => estateField({ region: "eu-central-1", estates: {} })).toThrow(/No estates configured/);
	});
});
