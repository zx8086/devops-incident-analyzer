...
import { registerCloudTrailTools } from "./cloudtrail/index.ts";
...
import { registerElbv2Tools } from "./elbv2/index.ts";
import { registerGuardDutyTools } from "./guardduty/index.ts";
...
import { registerRoute53Tools } from "./route53/index.ts";
import { registerS3Tools } from "./s3/index.ts";
import { registerSecurityHubTools } from "./securityhub/index.ts";
...
export function registerAllTools(server: McpServer, config: AwsConfig): void {
	...
	registerCloudTrailTools(server, config);
	...
	registerElbv2Tools(server, config);
	registerGuardDutyTools(server, config);
	...
	registerRoute53Tools(server, config);
	registerS3Tools(server, config);
	registerSecurityHubTools(server, config);
