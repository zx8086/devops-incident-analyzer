...
export const registerBulkIndexWithProgress = (server: McpServer, esClient: Client) => {
	const handler = async (
		toolArgs: BulkIndexWithProgressParams,
		...
		try {
			...
			for (let batchIndex = 0; batchIndex < batches; batchIndex++) {
				...
				try {
					// SIO-669: prefer top-level `operations` over deprecated `body` wrapper.
					const operations: Array<Record<string, unknown>> = [];
					for (const doc of batchDocs) {
						const indexOp: { index: { _index: string; pipeline?: string; routing?: string } } = {
							index: { _index: index },
						};
