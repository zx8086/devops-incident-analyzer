...
const reindexDocumentsValidator = z.object({
	...
	refresh: coerceBoolean.optional().describe("Refresh the destination index after completion"),
	...
	waitForCompletion: coerceBoolean.optional().describe("Wait for the operation to complete"),
...
function createReindexDocumentsMcpError(
	error: Error | string,
	context: { type: "validation" | "execution" | "index_not_found" | "index_already_exists"; details?: unknown },
): McpError {
...
export const registerReindexDocumentsTool: ToolRegistrationFunction = (server: McpServer, esClient: Client) => {
	const reindexDocumentsHandler = async (args: ReindexDocumentsParams): Promise<SearchResult> => {
		...
		try {
			...
			const result = await esClient.reindex({
				source: params.source as unknown as estypes.ReindexSource,
				dest: params.dest as unknown as estypes.ReindexDestination,
