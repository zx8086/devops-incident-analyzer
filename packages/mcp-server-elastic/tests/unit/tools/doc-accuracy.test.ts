...
describe("Documentation Accuracy", () => {
	...
	test("should document all available tools", async () => {
		...
		// Check coverage
		const documentedTools = Object.keys(generatedDocs.paths || {}).map((path) => path.replace("/tools/", ""));

		const coverageRatio = documentedTools.length / actualTools.length;

		console.log(
			`Documentation covers ${documentedTools.length}/${actualTools.length} tools (${(coverageRatio * 100).toFixed(1)}%)`,
		);
		...
		const missingTools = actualTools.filter((tool) => !documentedTools.includes(tool.name)).map((tool) => tool.name);
	...
	test("should have accurate parameter descriptions", () => {
		...
		if (parameterCount > 0) {
			...
			console.log(
				`Parameter descriptions: ${describedParameters}/${parameterCount} (${(descriptionRatio * 100).toFixed(1)}%)`,
			);
