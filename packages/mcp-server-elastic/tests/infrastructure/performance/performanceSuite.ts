...
class PerformanceTestSuite {
	...
	public compareWithBaseline(toolName: string): {
		...
		const durationChange =
			((current.summary.avgDuration - baseline.summary.avgDuration) / baseline.summary.avgDuration) * 100;
		const memoryChange =
			((current.summary.avgMemoryGrowth - baseline.summary.avgMemoryGrowth) / baseline.summary.avgMemoryGrowth) * 100;
	...
	public generateReport(): string {
		...
		report +=
			"| Tool | Category | Avg Duration (ms) | P95 Duration (ms) | Memory Growth (bytes) | Success Rate (%) |\n";
		report += "|------|----------|-------------------|-------------------|----------------------|------------------|\n";
...
export { type PerformanceBenchmark, type PerformanceMetrics, PerformanceTestSuite };
