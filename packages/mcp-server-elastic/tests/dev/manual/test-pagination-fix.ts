...
async function testPaginationUtility() {
	...
	console.log(
		`   Over-limit (5 items, limit 50): returned ${overLimitResult.results.length} of ${overLimitResult.metadata.total}`,
	);
...
async function testILMGetLifecycleScenario() {
	...
	console.log(
		`   Original bug - slice(0, undefined): returned ${undefinedLimitResult.length} policies (should be 92, not respecting limit)`,
	);
	...
	console.log(
		`   Fixed version - paginateResults with limit 50: returned ${fixedResult.results.length} of ${fixedResult.metadata.total}`,
	);
