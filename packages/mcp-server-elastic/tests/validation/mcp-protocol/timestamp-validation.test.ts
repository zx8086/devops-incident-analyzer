...
describe("Timestamp Validation Tests", () => {
	...
	test("search tool logs time range queries for debugging", async () => {
		...
		expect(testQuery.queryBody.query.range["@timestamp"]).toEqual(expect.objectContaining({ gte: "now-24h" }));
	});
	...
	test("aggregation results include metadata for debugging", async () => {
		...
		expect(mockAggregationResult.content[0].text).toEqual(expect.stringContaining("total hits"));
	});
