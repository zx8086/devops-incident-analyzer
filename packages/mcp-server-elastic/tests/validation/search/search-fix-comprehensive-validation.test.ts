...
describe("Time Range Query Tests", () => {
	test("Should support various time range formats", () => {
		...
		const timeRangeFormats = ["now-1h", "now-24h", "now-2h", "now-1d", "now-7d", "now-1M", "now-1h/h", "now-1d/d"];
...
describe("Request Format Generation Tests", () => {
	test("Should generate clean Elasticsearch request structure", () => {
		...
		const input = {
			...
			query: {
				bool: {
					must: [{ range: { "@timestamp": { gte: "now-24h" } } }, { term: { "service.name": "api-gateway" } }],
				},
			},
			size: 10,
			from: 0,
