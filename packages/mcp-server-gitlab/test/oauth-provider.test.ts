...
// Use a synthetic test instance URL so cleanup never touches the real
// `https___gitlab.com.json` a developer might have on disk.
const TEST_INSTANCE = "https://gitlab.test.invalid";
const TEST_FILE = join(STORAGE_DIR, "https___gitlab.test.invalid.json");
...
describe("GitLabOAuthProvider", () => {
	...
	test("clientMetadata uses public-client + PKCE per RFC 8252", () => {
		const provider = new GitLabOAuthProvider(TEST_INSTANCE, 9184, () => {});
	...
	test("redirectUrl uses the injected callback port", () => {
		const provider = new GitLabOAuthProvider(TEST_INSTANCE, 9555, () => {});
