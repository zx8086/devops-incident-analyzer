...
export function initializeTracing(options?: TracingOptions): void {
	const project = process.env.GITLAB_LANGSMITH_PROJECT || process.env.LANGSMITH_PROJECT || "gitlab-mcp-server";
...
export async function traceToolCall<T>(toolName: string, handler: () => Promise<T>): Promise<T> {
	...
	log.info({ tool: toolName, dataSource: "gitlab" }, `Tool call started: ${toolName}`);

	try {
		const result = await sharedTraceToolCall(toolName, handler, { dataSourceId: "gitlab" });
		const duration = Date.now() - startTime;
		log.info({ tool: toolName, dataSource: "gitlab", duration }, `Tool call completed: ${toolName}`);
		return result;
	} catch (error) {
		const duration = Date.now() - startTime;
		log.error(
