...
- Run a client: `just coms <name>` (sets pi `--name "<name> <timestamp>"`, `--cname <name>`, `--explicit`; extra args pass to pi); same-machine peers use a hub on `127.0.0.1` (`just coms-net-server`). `just coms` targets a LOCAL hub only -- it reads `PI_COMS_NET_*` from the ambient environment. For a DEPLOYED hub use `just coms-env <dev|prd> <name>`, which resolves url/token/project from `deploy/fleet.yaml` and needs that environment's tunnel open.
- `just hub-tunnel <dev|prd> [local-port]` takes an ENVIRONMENT, not a profile: profile, region and hub port come from `deploy/fleet.yaml` (`scripts/hub-tunnel-target.ts` resolves it, falling back to built-in defaults when the gitignored manifest is absent). Each environment derives its own local port (dev 8787, prd +1, stg +2) so several tunnels coexist; the hub instance is found by `tag:Name=pi-coms-hub`.
