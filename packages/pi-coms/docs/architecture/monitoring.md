...
| Spoke-health | 15 min | The hub's own agent listing, NOT an AWS API (SIO-1681) -- the only check that reads pi-coms. Each spoke counts its consecutive provider failures (`stopReason: "error"`, reset by any successful turn) and reports the count on every heartbeat; this raises a warn at three. No second cross-cycle gate, because the counter IS the persistence measure. Scoped to the monitor's own `INVESTIGATE_TARGET`, or one sick spoke would be reported by every monitor in the fleet. `status` deliberately stays `online`: it is a liveness axis the hub also derives from heartbeat age, so a 403ing spoke is reachable and simply not working. **Never investigated** -- every other family asks the account's agent to diagnose what it observes, but here the agent IS the subject and its model calls are the thing failing, so the finding ships to the report undiagnosed rather than spending a slot on a prompt that cannot be answered. That exclusion is not the whole boundary, though: the finding is journaled whole and `priorIncidents` matches rows by a bare `payload LIKE '%resource%'` across every family, so a later finding on the same spoke would carry this one into an investigation prompt. Both spoke-reported strings are therefore bounded before they enter the finding -- the provider error is reduced to a CLASS (`access-denied`/`throttled`/`timeout`/`other`/`unknown`) and the model id must match a model-id shape or reads `unreported`. Classified, not sanitised: an allowlist cannot smuggle anything, while an escaping rule is one missed case from failing. The operator reads the full message from `/v1/agents` or the host. Caveat: the counter only moves on runs that happen, so an idle spoke accumulates nothing | Spoke + `model-errors`, 24 h re-alert, clears as soon as the counter resets |
