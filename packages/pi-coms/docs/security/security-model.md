...
| `pi-coms-dev-extensions` (inline) | Named dev additions: `ce:GetCostAndUsage` (cost check), Bedrock invoke on Anthropic models, scheduling/history reads, `LogContentReads` (FilterLogEvents/GetLogEvents/StartQuery/GetQueryResults), `CertificateReads` (acm:List/DescribeCertificate for the cert-expiry check), `AmplifyReads` (amplify:ListApps, ListJobs/GetJob plus app, branch, and webhook reads for build-history checks), `DriftAndComplianceReads` (SIO-1674: CloudFormation `DetectStackDrift`/`DetectStackResourceDrift`/`DescribeStackDriftDetectionStatus` and stack-set reads, AWS Config per-resource, summary, conformance-pack and aggregator compliance reads, and the IAM role, policy and instance-profile reads drift detection needs on Control Tower stacks (SIO-1677; no user, credential or access-key reads); the `Detect*Drift` actions start an evaluation and are classified non-write by the service authorization reference), and an explicit **Deny** on secret values, `kms:Decrypt`, SSM parameter values, `lambda:GetFunction`, and data-plane gets -- metadata-only made structural |
