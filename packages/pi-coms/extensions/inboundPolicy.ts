...
// A refusal reason starts with "refused:" so the sender can tell it from a
// failed turn (the monitor's budget does not count refusals).
...
export function decideInbound(f: InboundFacts): InboundDecision {
	...
	for (const pattern of f.mutePatterns) {
		if (globToRegExp(pattern).test(f.senderName)) {
			return { kind: "refuse", reason: `refused: recipient muted (${pattern})` };
		}
	}
	...
	if (f.hasSchema && f.contextPct !== null && f.contextPct >= f.refuseAbovePct) {
		return {
			kind: "refuse",
			reason: `refused: recipient context at ${Math.round(f.contextPct)}%, refusing investigation`,
		};
	}
	return { kind: "turn" };
}
