...
export const ConfigSchema = z.object({
  ...
  kong: z
    .object({
      accessToken: z
        .string()
        .min(1, "Kong Konnect access token is required")
        .describe("Kong Konnect API access token"),
      region: z
        .enum(["us", "eu", "au", "me", "in"])
        .default("us")
        .describe("Kong Konnect API region"),
      baseUrl: z
        .string()
        .url()
        .optional()
        .describe(
          "Custom Kong API base URL (auto-generated from region if not provided)",
        ),
      timeout: z
        .number()
        .min(1000)
  ...
  tracing: z
    .object({
      ...
      apiKey: z
        .string()
        .optional()
        .describe("LangSmith API key (required when tracing is enabled)"),
      project: z
        .string()
        .default("konnect-mcp-server")
        .describe("LangSmith project name"),
      endpoint: z.string().url().default("https://api.smith.langchain.com"),
      sessionName: z
        .string()
        .default("mcp-session")
        .describe("Session name for tracing"),
      tags: z
        .array(z.string())
        .default(["mcp-server", "kong-konnect"])
        .describe("Tags for tracing sessions"),
      samplingRate: z
        .number()
        .min(0)
        .max(1)
        .default(1.0)
        .describe("Sampling rate for traces (0.0 to 1.0)"),
    })
      ...
      performanceThresholds: z
        .object({
          ...
          errorRate: z
            ...
            .max(100)
            .default(5)
            .describe("Error rate threshold as percentage"),
        })
  ...
  runtime: z
    .object({
      preferBunEnv: z
        .boolean()
        .default(true)
        .describe("Prefer Bun.env over process.env when available"),
      envFileAutoLoad: z
        .boolean()
        .default(true)
        .describe("Auto-load .env files (Bun does this automatically)"),
      debugMode: z
        .boolean()
        .default(false)
        .describe("Enable debug mode for additional logging"),
    })
...
export class ConfigurationHealthMonitor {
  ...
  async assessConfigurationHealth(
    ...
    try {
      ...
      health.metrics.environmentConsistency =
        this.assessEnvironmentConsistency(config);
      ...
      health.metrics.validationPerformance = Math.max(
        Date.now() - startTime,
        1,
      );
  ...
  private calculateSecurityScore(config: Config): number {
    ...
    if (
      config.kong.accessToken === "your-token-here" ||
  ...
  private analyzeConfigurationIssues(
    ...
    if (config.tracing.enabled && !config.tracing.apiKey) {
      health.issues.critical.push({
        ...
        severity: "critical",
        remediation:
          "Set LANGSMITH_API_KEY environment variable or disable tracing",
      });
    }
    ...
    if (
      ...
      config.tracing.apiKey &&
      !config.tracing.apiKey.startsWith("lsv2_")
    ) {
      health.issues.warnings.push({
        path: "tracing.apiKey",
  ...
  private generateRecommendations(
    ...
    if (health.metrics.securityScore < 80) {
      health.recommendations.push(
        "Review security configuration - score below 80%",
      );
    }

    if (config.kong.timeout > 30000) {
      health.recommendations.push(
        "Consider reducing Kong API timeout for better responsiveness",
      );
    }
    ...
    if (config.application.environment === "production") {
      if (health.metrics.environmentConsistency < 90) {
        health.recommendations.push(
          "Configuration not optimized for production environment",
        );
      }
      if (!config.monitoring.enabled) {
        health.recommendations.push(
          "Enable monitoring in production for better observability",
        );
      }
    }
    if (config.application.environment === "development") {
      if (config.application.logLevel !== "debug") {
        health.recommendations.push(
          "Consider debug logging in development for better troubleshooting",
        );
      }
    }
    ...
    if (
      ...
      health.recommendations.push(
        "Consider reducing sampling rate in production to improve performance",
      );
    }
    if (
      !config.tracing.enabled &&
      ...
      health.recommendations.push(
        "Consider enabling tracing in non-production environments for debugging",
      );
    }

    if (config.kong.retryAttempts === 0) {
      health.recommendations.push(
        "Consider enabling retry attempts for improved resilience",
      );
    }
    if (config.kong.retryDelay < 500) {
      health.recommendations.push(
        "Consider increasing retry delay to avoid overwhelming the API",
      );
    }
  }
  private determineHealthStatus(
    ...
    if (
      ...
      health.metrics.securityScore < 80
    ) {
      return "degraded";
    }
    return "healthy";
    ...
    return {
      ...
      kong: {
        ...
        retryAttempts: parseInt(
          getEnvVarWithDefault("KONNECT_RETRY_ATTEMPTS", "3"),
        ),
        retryDelay: parseInt(
          getEnvVarWithDefault("KONNECT_RETRY_DELAY", "1000"),
        ),
      },
      tracing: {
        enabled: getEnvVarWithDefault("LANGSMITH_TRACING", "false") === "true",
        apiKey: getEnvVar("LANGSMITH_API_KEY"),
        project: getEnvVarWithDefault(
          "LANGSMITH_PROJECT",
          "konnect-mcp-server",
        ),
        endpoint: getEnvVarWithDefault(
          "LANGSMITH_ENDPOINT",
      ...
      monitoring: {
        ...
        healthCheckInterval: parseInt(
          getEnvVarWithDefault("HEALTH_CHECK_INTERVAL", "30000"),
        ),
        metricsCollection:
          getEnvVarWithDefault("METRICS_COLLECTION", "true") === "true",
        performanceThresholds: {
          responseTimeMs: parseInt(
            getEnvVarWithDefault("PERFORMANCE_RESPONSE_TIME_MS", "5000"),
          ),
          errorRate: parseFloat(
            getEnvVarWithDefault("PERFORMANCE_ERROR_RATE", "5"),
          ),
        },
      },
      runtime: {
        ...
        envFileAutoLoad:
          getEnvVarWithDefault("ENV_FILE_AUTO_LOAD", "true") === "true",
  ...
  private handleValidationError(error: z.ZodError): void {
    ...
    error.issues.forEach((issue) => {
      ...
      if (
        ...
        mcpLogger.warning(
          "config",
          "TIP: Set LANGSMITH_API_KEY environment variable or set LANGSMITH_TRACING=false",
        );
      }
    });
  }
