...
export function cleanupSession(sessionId: string) {
	...
	if (session?.startTime) {
		...
		const conversationDuration = session.conversationStartTime ? Date.now() - session.conversationStartTime : duration;
 ...
 */
export function updateSessionWithConversation(toolName: string, intent?: string, topic?: string): void {
	const session = getCurrentSession();
	if (!session) return;
