...
import type { ConversationFlow, ConversationInfo } from "./conversation-tracker.js";
...
function assessEngagementLevel(conversation: ConversationInfo): "low" | "medium" | "high" {
	...
	if (messageCount > 10 || new Set(toolSequence).size > 6 || duration > 5 * 60 * 1000) {
		// 5+ minutes
		return "high";
	}

	// Medium engagement indicators
	if (messageCount > 3 || new Set(toolSequence).size > 2 || duration > 60 * 1000) {
		// 1+ minute
		return "medium";
	}
...
function assessComplexityLevel(conversation: ConversationInfo): "simple" | "moderate" | "complex" {
	...
	// Complex indicators
	if (uniqueTools > 8 || contextSwitches > 3 || topics.length > 4 || hasAdvancedToolUsage(toolSequence)) {
		return "complex";
	}
