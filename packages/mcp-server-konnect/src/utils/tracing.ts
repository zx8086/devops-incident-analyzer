...
import { detectContextTransition, detectIntent, type UserIntent } from "./intent-detector.js";
...
export class UniversalTracingManager {
	...
	private async initializeLangSmith(): Promise<void> {
		try {
			...
			mcpLogger.warning("tracing", "LangSmith initialization failed - graceful degradation active", {
				error: error.message,
			});
			this.enabled = false;
		}
				...
				async (toolInput: any) => {
					...
					try {
						...
						if (session?.sessionId && conversation) {
							const updatedConversation = updateConversation(session.sessionId, toolName, executionTime, success);
	...
	createToolTracer<T extends any[], R>(
		toolName: string,
	): (operation: (...args: T) => Promise<R>, metadata?: TraceMetadata) => Promise<R> {
		return async (
	...
	async getConversationInsights(): Promise<{
		...
		const quality = analyzeConversationQuality(conversationStats.conversation, undefined, conversationStats.flow);
