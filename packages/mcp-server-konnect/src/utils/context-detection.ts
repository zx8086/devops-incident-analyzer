...
export interface DetectionPattern {
	...
	pattern: RegExp;
	confidence: number;
	extractor: (match: RegExpMatchArray) => string | undefined;
...
export class ContextDetector {
	...
	private normalizeEnvironment(env: string | undefined): string {
		if (!env) return "";
	...
	private calculateSummary(result: DetectionResult): void {
		...
		const bestSignals = [result.summary.bestDomain, result.summary.bestEnvironment, result.summary.bestTeam].filter(
			Boolean,
		) as ContextSignal[];

		result.summary.overallConfidence =
			bestSignals.length > 0 ? bestSignals.reduce((sum, signal) => sum + signal.confidence, 0) / bestSignals.length : 0;
	}
