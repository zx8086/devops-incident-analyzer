...
export async function getConsumerRequests(
	...
	return withErrorContext(
		...
		const serviceBreakdown: Record<string, { count: number; statusCodes: Record<string, number> }> = {};

		if (result.results.length > 0) {
			// Calculate average latency
			avgLatency =
				result.results.reduce((sum, req) => sum + (req.latencies_response_ms || 0), 0) / result.results.length;
