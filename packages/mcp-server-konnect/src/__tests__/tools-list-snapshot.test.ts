...
// SIO-1414: eval-safety lock for the sugar->registerTool sweep (series template,
// C-1..C-8). The serialized tools/list -- name, description, inputSchema -- is the
...
// inline (they are new in this sweep and reviewable in PR diffs -- clients read
// them as hints, the model never sees them).
//
...
const fakeConfig = { application: { name: "kong-konnect-mcp", version: "2.0.0" } } as unknown as Config;
	...
	if (drifted.length > 0) {
		throw new Error(
			`LLM-visible surface (description/inputSchema serialization) drifted for: ${drifted.join(", ")}. ` +
				"If intentional, regenerate with REGEN_TOOLS_SNAPSHOT=1 bun test tools-list-snapshot and review the fixture diff (SIO-1414).",
		);
	}
