...
describe("Flight API Security Tests", () => {
	...
	describe("Authentication and Authorization", () => {
		...
		test("should validate API key permissions", async () => {
			...
			for (const endpoint of restrictedEndpoints) {
				const response = await testUtils.simulateApiRequest(endpoint.method, endpoint.path, {
	...
	describe("Input Validation and Sanitization", () => {
		test("should reject SQL injection attempts", async () => {
			...
			for (const payload of sqlInjectionPayloads) {
				...
				const postResponse = await testUtils.simulateApiRequest("POST", "/flights", headers, {
					airline: payload,
					origin: "JFK",
				});
		...
		test("should prevent XSS attacks", async () => {
			...
			for (const payload of xssPayloads) {
				const response = await testUtils.simulateApiRequest("POST", "/flights", headers, {
					airline: payload,
					description: payload,
					origin: "JFK",
					destination: "LAX",
		...
		test("should validate data types and formats", async () => {
			...
			for (const input of invalidInputs) {
				...
				const response = await testUtils.simulateApiRequest("POST", "/flights", headers, requestBody);
		...
		test("should handle oversized payloads", async () => {
			const oversizedData = {
				...
				metadata: Object.fromEntries(Array.from({ length: 100 }, (_, i) => [`key-${i}`, `value-${i}`])), // Too many metadata fields
			};

			const headers = { "X-API-Key": TEST_FIXTURES.authCredentials.keyAuth };

			const response = await testUtils.simulateApiRequest("POST", "/flights", headers, oversizedData);
	...
	describe("Data Privacy and Security", () => {
		test("should not expose sensitive data in responses", async () => {
			const sensitiveFields = ["password", "secret", "token", "private", "ssn", "creditCard", "bankAccount", "apiKey"];
