...
const SCHEMA_CARD = [
	...
	"Landing Zone repository subgraph (GitLab numeric IDs remain stable across path renames):",
	"  GitLabGroup(id, path, name, webUrl, lastSyncedAt)  Repository(id, groupId, path, name, defaultBranch, webUrl, commitSha, lastSyncedAt)",
	"  TerraformRoot(id, repositoryId, path, managesAccounts, lastSyncedAt)  TerraformModule(id, repositoryId, path, name, lastSyncedAt)",
	"  SharedModule(id, source, version, lastSyncedAt)  TerraformPlan(id, status, summary, artifactUrl, createdAt)",
	"  Standard(id, title, status, url)  ADR(id, title, status, url)",
	"  MergeRequest.url is the stable '<project-id>:<iid>' key for Landing Zone imports; webUrl is the mutable browser URL.",
	"  (GitLabGroup)-[:CONTAINS]->(Repository)-[:REPOSITORY_CONTAINS_ROOT]->(TerraformRoot)",
	"  (TerraformRoot)-[:ROOT_USES_MODULE]->(TerraformModule)-[:MODULE_USES_SHARED_MODULE]->(SharedModule)",
	"  (ConfigChange)-[:CHANGE_TARGETS_REPOSITORY]->(Repository)  (ConfigChange)-[:CHANGE_TARGETS_ROOT]->(TerraformRoot)",
	"  (Pipeline)-[:PRODUCED]->(TerraformPlan)  (Repository)-[:GOVERNED_BY]->(Standard)-[:IMPLEMENTS]->(ADR)",
