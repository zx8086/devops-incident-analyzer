...
describe("SIO-1864: action_keywords coverage", () => {
	...
	test("the eval dataset queries keyword-match their datasource", () => {
		...
		const cases: Array<[string, string, string]> = [
			["couchbase-cluster-health", "What is the current health of the Couchbase Capella cluster hosting the default bucket", "system_vitals"],
			["couchbase-cluster-health", "Give me a full query-performance profile of the Couchbase cluster behind the default bucket", "expensive_queries"],
			["elastic-search-logs", "Search the last 24 hours of logs on eu-b2b for the pvh-services-styles-v3 service and report the error volume", "search"],
			["elastic-search-logs", "What is the current cluster health of the eu-b2b deployment? Report status, node count, and any unassigned shards", "cluster_health"],
			["kafka-introspect", "Describe the c72-shared-services-msk Kafka cluster: how many brokers are there and what is the controller?", "cluster_info"],
			["kafka-introspect", "List the consumer groups and report which ones are showing meaningful lag", "consumer_lag"],
			["gitlab-api", "List the most recent merge requests in GitLab project 43242609 and summarise what changed", "merge_requests"],
			["atlassian-api", "Search Jira for recent incident tickets and summarise the most recent few", "incident_correlation"],
			["konnect-api-gateway", "List the services configured in Kong Konnect and report their upstream targets", "service_config"],
		];
		const misses: string[] = [];
		for (const [toolName, query, expected] of cases) {
			...
			if (!matched.includes(expected)) misses.push(`${toolName}: expected ${expected}, got [${matched}] for "${query.slice(0, 50)}..."`);
		}
