...
type QueuedWrite = { kind: "fact"; text: string } | { kind: "message"; message: ChatMessageBlock; ttlSeconds?: number };
...
export function enqueueFact(text: string): void {
	queue.push({ kind: "fact", text });
	maybeFlush();
}

export function enqueueMessage(message: ChatMessageBlock, ttlSeconds?: number): void {
	queue.push({ kind: "message", message, ttlSeconds });
	maybeFlush();
}
...
// (idempotent, 409-tolerant). Best-effort: on error the batch is logged and
// dropped — live memory must never block or fail a session.
export async function flushAgentMemory(): Promise<void> {
	...
	try {
		...
		const facts = batch
			.filter((w): w is Extract<QueuedWrite, { kind: "fact" }> => w.kind === "fact")
			.map((w) => w.text);
		if (facts.length > 0) await c.addFacts(ref, facts);
		for (const w of batch) {
			if (w.kind === "message") await c.addMessages(ref, [w.message], w.ttlSeconds);
		}
		logger.info({ facts: facts.length, total: batch.length }, "flushed agent-memory writes");
	} catch (error) {
		logger.warn(
