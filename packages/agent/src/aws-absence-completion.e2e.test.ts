...
describe("SIO-1784 end to end through the real closure", () => {
	test("a lazily-enumerated estate is completed and the proof holds", async () => {
		...
		const listClusters = tool(
			async () => JSON.stringify({ clusterArns: CLUSTERS.map((c) => `arn:aws:ecs:eu-central-1:1:cluster/${c}`) }),
		...
		const listServices = tool(async () => JSON.stringify({ serviceArns: [] }), {
			...
			description: "d",
			schema: z.object({ cluster: z.string(), cursor: z.string().optional() }),
		});

		const runSignals: {
			serviceAbsent: boolean;
			absenceBlockedBy?: string | null;
			observeEcsPage?: (t: string, c: unknown, a: unknown) => void;
		} = { serviceAbsent: false };
		...
		const wrapped = instrumentTools([listClusters, listServices], {
			...
			focusServices: ["prana-import-export-service"],
			runSignals,
		} as never);
		...
		await byName
			...
			?.invoke({ id: "s", name: "aws_ecs_list_services", args: { cluster: CLUSTERS[0] }, type: "tool_call" });
