...
export function getToolsForDataSource(dataSourceId: string): StructuredToolInterface[] {
  ...
  return allTools.filter((tool) => {
    ...
    if (dataSourceId === "couchbase") return name.includes("couchbase") || name.includes("sql_plus") || name.includes("get_system") || name.includes("get_fatal") || name.includes("get_longest") || name.includes("get_most");
    if (dataSourceId === "konnect") return name.includes("konnect") || name.includes("control_plane") || name.includes("portal") || name.includes("service") || name.includes("route") || name.includes("plugin") || name.includes("consumer") || name.includes("certificate");
    return false;
  });
}
