...
export interface IacReview {
	...
	// SIO-1083: three-state status per recall source ("off" | "empty" | "populated"), so the card
	// tells a disabled backend from an enabled-but-cold one. Same silent-drop gotcha as above: the
	// server sets these on the review payload, so they MUST exist here or the card drops them.
...
export interface LandingZoneClarifyPrompt {
	threadId: string;
	question: string;
}
...
export interface ReducerState {
	...
	landingZoneClarify: LandingZoneClarifyPrompt | null;
...
export function applyStreamEvent(state: ReducerState, event: StreamEvent): ReducerState {
	switch (event.type) {
		...
		case "landing_zone_clarify":
			return {
				...state,
				threadId: event.threadId,
				landingZoneClarify: { threadId: event.threadId, question: event.question },
			};
		case "landing_zone_clarify_resolved":
			return { ...state, landingZoneClarify: null };
