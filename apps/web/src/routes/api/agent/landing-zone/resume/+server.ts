...
const ReviewResumeRequestSchema = z.discriminatedUnion("decision", [
...
const ResumeRequestSchema = z.union([
	ReviewResumeRequestSchema,
	z.object({ threadId: z.string().min(1), answer: z.string().trim().min(1).max(500) }).strict(),
]);
...
export const POST: RequestHandler = async ({ request }) => {
	...
	const pendingType =
		typeof pending?.value === "object" && pending.value !== null
			? (pending.value as { type?: unknown }).type
			: undefined;
	if (
		!pending ||
		("answer" in body ? pendingType !== "landing_zone_clarify" : pendingType !== "landing_zone_plan_review")
	) {
		return json({ error: "No matching Landing Zone gate is pending for this thread" }, { status: 409 });
	}
	const pendingReviewId = (pending.value as { review?: { reviewId?: unknown } }).review?.reviewId;
	if (!("answer" in body) && pendingReviewId !== body.reviewId)
			...
			await runWithRequestContext({ threadId: body.threadId, runId, requestId }, async () => {
				...
				try {
					await traceSpan(
						...
						async () => {
							send({ type: "answer" in body ? "landing_zone_clarify_resolved" : "landing_zone_review_resolved" });
